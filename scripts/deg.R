...
# Merge counts with gene coordinates
...
# ------------- calcualte average and save --------------
# sort
dd <- as.data.frame(normalized_counts_not_filter)
dd <- dd[,order(names(dd))]
# calculate means
n <- 3
i <- seq(1, length(dd), n)

dd2 <- data.frame(nut = rownames(dd))
dd2[, paste0('NewCol', seq_along(i))] <- lapply(i, function (j) rowMeans(dd[, j:min(j+2, length(dd))]))
rownames(dd2) <- dd2$nut
dd2$nut <- NULL

# add colnames
newnames <- c()
for (j in 1:length(i)){
        name = str_split(names(dd)[i[j]], pattern="_")[[1]][1]
        newnames[j] = name
}
names(dd2) <- newnames

# save
write.table(format(dd2, digits=2, nsmall=2), file="avg_not_filtered_normalized_counts.tsv", sep="\t", quote=F, col.names=NA)
# ------------- calcualte average and save --------------
...
# sort
gnc2 <- g_nc
dd <- as.data.frame(gnc2)
rownames(dd) <- dd$Row.names
dd <- dd[,order(names(dd))]
epril_names <- names(dd)[grepl("epiRIL", names(dd))]
ddg <- dd[c("Chr", "Start", "End", "Strand", "Length")]
ddc <- dd[epril_names]
# calculate means
dd <- ddc
n <- 3
